...
public class userboardDaoImpl implements userboardDao{
	...
	public List<userboardBean> selectAllBoards(String field, String query, int page) {
		...
		String sql = "SELECT * "
				+ "FROM (SELECT @ROWNUM:=@ROWNUM+1 AS rownum, userboard.* "
				+ "FROM userboard, (SELECT @ROWNUM := 0) a1 "
				+ "where "+field+" like ? "
				+ "ORDER BY now() ASC) a2 "
				+ "where a2.rownum between ? and ? "
				+ "ORDER BY a2.rownum DESC";
		try {
	...
	public void insertBoard(userboardBean board) {
		String sql = "INSERT INTO userboard (title, userName, password, regDate, readcount, content)" + "VALUES (?, ?, ?, now(), ?, ?)";
