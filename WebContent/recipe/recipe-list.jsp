...
<html lang="ko">
  ...
  <body>
    ...
    <!-- 본문 -->
      ...
      <h3 class="text-center mt-3"><c:out value="${categoryName}"/>게시판</h3>
      <hr />
        ...
        <tbody>        
          ...
          <c:forEach var="recipe" items="${listRecipe}">
            <tr>
              ...
              <td><a href="<%=request.getContextPath()%>/recipes?action=show&id=${recipe.recipe_id}" style="color:black; text-decoration:none;" ><c:out value="${recipe.recipeName}" /></a></td>
              <td><c:out value="${recipe.userName}" /></td>
              <td><c:out value="${recipe.regDate}" /></td>       	
              <td><c:out value="${recipe.viewCount}" /></td>       	
              <c:if test="${who == 'admin'}">
                <td>
                  <a href="<%=request.getContextPath()%>/recipes?action=delete&id=${recipe.recipe_id}" onclick="return confirm('정말 삭제하시겠습니까?');" class="btn btn-danger btn-sm"> 삭제 </a>
                </td>
              </c:if>              
            </tr>
          </c:forEach> 
